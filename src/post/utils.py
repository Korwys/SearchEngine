...
def transform_data_from_file_to_db() -> None:
    ...
    df_new.to_sql('post', db, if_exists='append', index=False, )
...
def main() -> None:
    try:
        ...
        if manager.elastic_server.indices.exists(index='post'):
            manager.elastic_server.indices.delete(index='post')
        manager.elastic_server.indices.create(index='post')
